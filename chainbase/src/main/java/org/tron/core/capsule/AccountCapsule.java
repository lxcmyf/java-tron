...
public class AccountCapsule implements ProtoCapsule<Account>, Comparable<AccountCapsule> {
  ...
  public boolean addAssetAmount(byte[] key, long amount) {
    ...
    this.account = this.account.toBuilder().putAsset(nameKey, addExact(currentAmount, amount))
        .build();
    return true;
  }
  ...
  public boolean addAssetAmountV2(byte[] key, long amount,
    ...
    if (dynamicPropertiesStore.getAllowSameTokenName() == 0) {
      ...
      this.account = this.account.toBuilder()
          .putAsset(nameKey, addExact(currentAmount, amount))
          .putAssetV2(tokenID, addExact(currentAmount, amount))
          .build();
    }
    //key is token id
    if (dynamicPropertiesStore.getAllowSameTokenName() == 1) {
      ...
      this.account = this.account.toBuilder()
          .putAssetV2(tokenIDStr, addExact(currentAmount, amount))
          .build();
    }
    return true;
  }
  ...
  public boolean reduceAssetAmount(byte[] key, long amount) {
    ...
    if (amount > 0 && null != currentAmount && amount <= currentAmount) {
      this.account = this.account.toBuilder()
              .putAsset(nameKey, subtractExact(currentAmount, amount)).build();
      return true;
    }
  ...
  public boolean reduceAssetAmountV2(byte[] key, long amount,
    ...
    if (dynamicPropertiesStore.getAllowSameTokenName() == 0) {
      ...
      if (amount > 0 && null != currentAmount && amount <= currentAmount) {
        this.account = this.account.toBuilder()
                .putAsset(nameKey, subtractExact(currentAmount, amount))
                .putAssetV2(tokenID, subtractExact(currentAmount, amount))
                .build();
        return true;
      }
    }
